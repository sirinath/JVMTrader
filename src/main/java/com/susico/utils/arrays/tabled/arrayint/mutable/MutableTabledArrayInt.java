...
public abstract class MutableTabledArrayInt extends ImmutableTabledArrayInt {
    protected MutableTabledArrayInt(final boolean checked, final int definedAsValues, final int length, final int ... values) {
        super(checked, definedAsValues, length, values);
    }


    public abstract void put(final int index, final int value);

    protected final void putToRest(final int index, final int value) {
        ARRAY_ACCESS.put(rest, index - definedAsValues, value);
    }

    public static  MutableTabledArrayInt getInstance(final boolean checked, final int length, final int ... values) {
        switch (length) {
            case 0:
            ...
            case 1024: 
                return MutableTabledArray1024Int.getInstance(checked, length, values);
