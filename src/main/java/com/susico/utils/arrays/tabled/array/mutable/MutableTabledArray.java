...
public abstract class MutableTabledArray<T> extends ImmutableTabledArray<T> {
    protected MutableTabledArray(final boolean checked, final int definedAsValues, final int length, final T ... values) {
        super(checked, definedAsValues, length, values);
    }


    public abstract void put(final int index, final T value);

    protected final void putToRest(final int index, final T value) {
        ARRAY_ACCESS.put(rest, index - definedAsValues, value);
    }

    public static <T> MutableTabledArray<T> getInstance(final boolean checked, final int length, final T ... values) {
        switch (length) {
            case 0:
            ...
            case 1024: 
                return MutableTabledArray1024.<T>getInstance(checked, length, values);


            default:
                return MutableTabledArray1024.getInstance(checked, length, values);
        }
    }
}